...
int main() {
	...
	LPVOID RemoteString = VirtualAllocEx(process, nullptr, strlen(full_path), MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE); 

	WriteProcessMemory(process, RemoteString, full_path, strlen(full_path), nullptr); 

	auto thread = CreateRemoteThread(process, nullptr, 0, LoadLib, RemoteString, NULL, nullptr); 	
	if (!thread) {
